...
- 2026-03-19：阶段 2 完成（**历史编号**：Demo 1 红移拉伸：InstancedMesh + aRedshift + uniform 动画；Demo 3 模态 Inpaint：shader 扫描线扩散 + 点击选点）
...
- 2026-03-23：Demo 口径对齐（021 Copilot 预置补齐为 4 个 Demo：CSST/Redshift/GOTTA/Inpaint）；补齐 GOTTA 示例数据（public JSON）；修复 macro→micro 切换后 micro 场景“看不见”的相机/controls 同步问题。

编号说明（避免 Demo 混用）：
- 2026-03-23 起：**021 Copilot 的 Command Palette 文案与预置列表统一采用“新的四大任务=四个 Demo（Demo1–Demo4）”口径**：
    - Demo 1：CSST Galaxy Decompose
    - Demo 2：Redshift Burst
    - Demo 3：GOTTA Transient Capture
    - Demo 4：Modal Inpaint
- 兼容说明：早期 Stage 2 历史口径中，Redshift Burst 曾标为 Demo 1、Modal Inpaint 曾标为 Demo 3。若需要对照旧编号：旧 Demo 1 ≈ 新 Demo 2；旧 Demo 3 ≈ 新 Demo 4。
...
阶段 2 (视觉核心与宏观 Demo 1–4 MVP - 1周)

利用 Three.js PostProcessing 实现高品质 Bloom（泛光）。

跑通以下宏观演示闭环（前端本地确定性为主，后端接口可先 Mock）：

- Demo 1：CSST Galaxy Decompose
- Demo 2：Redshift Burst
- Demo 3：GOTTA Transient Capture（最小可视化闭环 + 示例数据）
- Demo 4：Modal Inpaint
...
## 阶段 2（Demo 1–4）端到端验收：以 Docker Dev 为准
...
2. 选择预置：`[v7.5] OneAstronomy · Redshift Burst (Demo 2)`
...
1. 在 Command Palette 选择：`[v7.5] OneAstronomy · Modal Inpaint (Demo 4)`
...
### 4.5) Demo 3：GOTTA Transient Capture（瞬变源捕获）验收

触发方式（推荐）：

1. 在 Command Palette 选择：`[v7.5] OneAstronomy · GOTTA Transient Capture (Demo 3)`

数据约定（前端直读 public）：

- `/data/astronomy/gotta_transient_event.json`（对应仓库文件：`frontend/public/data/astronomy/gotta_transient_event.json`）

预期现象：

- Sky（macro）中出现一个“瞬变源标记”（Additive 发光 + 脉冲闪烁），并有轻微的视角聚焦（相机/target 指向标记）。
- 触发 GOTTA 时必须遵守场景霸权：清理/退出 CSST overlay、收缩 redshift 爆裂、退出 inpaint（避免多叠层污染）。
...
目标：在保持 Stage 2（Demo 1–4 的宏观演示）稳定可验收的前提下，推进 Stage 3（Demo 2 & Demo 4 的硬核联调/算力验证）并补齐“工程化可回归”的验收机制。
