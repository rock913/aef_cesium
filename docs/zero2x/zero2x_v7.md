...
- ✅ Milestone 3（HUD / LayerTree 跨模态映射）已完成：
  - LayerTree 分组与参数面板：Earth vs Three
    - 实现：`frontend/src/views/workbench/components/LayerTree.vue`
    - 新增 Three 图层：`bloom` / `macro-spiral` / `micro-atoms`
  - Three 参数映射（纯函数，可测）：
    - 实现：`frontend/src/views/workbench/engines/threeLayerMapping.js`
    - 测试：`frontend/tests/threeLayerMapping.test.js`
  - 引擎网关保护（不破坏 v6）：
    - `EngineScaleRouter` 在 `earth` 模式仅透传允许的 GEE/矢量 layer，避免 Three 专用 layer 被 v6 引擎误处理

- ✅ Milestone 4（EXECUTE ON TWIN：TaskQueue + scale 分发）已完成：
  - 串行任务队列：`frontend/src/utils/taskQueue.js`
    - 测试：`frontend/tests/taskQueue.test.js`
  - Workbench `runExecute()` 按 `currentScale` 分发：
    - `earth`：沿用既有后端 stats/analyze/report
    - `macro`：ThreeTwin 动作（高亮簇 + 环绕镜头）
    - `micro`：ThreeTwin 动作（晶格重组动画）
  - ThreeTwin 暴露动作接口（供 EXECUTE 调用）：
  ...
  - OmniCommand 的 D/E 剧本目前复用既有 contextId（演示以 scale + layers + EXECUTE 动作为主），真实 gaia/protein 数据与后端计算仍在 Future Phase。
  - Earth 模式对 layers 做了白名单过滤：Three 专用 layers 不会进入 v6 Cesium 引擎（这是“绝不破坏 v6”的硬约束）。
  - Three 端的“微观重组/宏观高亮”是动作脚本（用于验证 EXECUTE 分发与 TaskQueue 稳定性），后续可替换为真实数据驱动（shader/compute/后端结果）。
...
as-built 注记：当前实现以“可回归的引擎动作脚本”为主（ThreeTwin 内部动作 + TaskQueue 串行），为后续接入真实天文/分子数据与后端计算保留接口与演进空间。

推荐的下一步（从可演示走向可研究）

1) 将 Three 层的 schema 固化为“版本化 layers contract”（含默认值与迁移），避免 sessionStorage 历史数据造成参数缺失。
2) 抽象 “Twin Actions” 协议：把 `highlight/spin/rebuild` 迁移为明确的 action list（带参数/校验/日志），用于接入 LLM 结构化工具调用。
3) 引入真实数据最小闭环：
  - Macro：加载小规模星表（几千点）+ 简单聚类结果可视化
  - Micro：加载一个小 PDB（或后端解析后的 positions/bonds）替换当前 instanced jitter
...
Milestone 4: 演进至真实数据验证 (Future Phase 2.5) ✅（以“动作脚本 + 分发/队列稳定性”为验收口径）

Milestone 5: 真实数据最小闭环 (Next)

- Macro：小星表加载 + 聚类结果可视化（替换/叠加过程化星系）
- Micro：小 PDB/.cif 数据闭环（后端解析 -> 前端渲染 bonds/atoms）
- Agent：将 EXECUTE 输出升级为结构化 action list，映射到 Twin Actions 协议
