...
#### 0303 视觉升级处方：高逼真度“数学合成风场”（立即见效）

目标：在**不接入真实风场纹理**（GFS/ERA5）且不改前端绑定的前提下，通过 WGSL 的物理与渲染逻辑，直接合成出“看起来像真实气象”的全球流体效果。

视觉升级核心（从低风险到高表现，建议按顺序叠加）：
- 速度色谱（Color Ramp）：依据流速映射颜色（蓝 → 青 → 黄/绿 → 红），让气旋/急流“一眼可见”。
- 气候带（Zonal Bands）：使用纬度函数模拟赤道信风带与中纬度西风带，快速形成全球带状结构。
- 气旋注入（Cyclones）：手动注入 2~3 个 swirl 旋涡作为视觉焦点（红色台风眼）。
- 渐隐拉丝（Fade Trails）：尾部完全透明、头部半透明；叠加 overlay 的 fade pass 形成发光发丝感。
...
参考实现：见 [docs/zero2x/update_patch_0303.md](docs/zero2x/update_patch_0303.md) 的“数学合成风场”全量 WGSL（含 compute+render、速度色谱、气旋与渐隐拉丝）。
