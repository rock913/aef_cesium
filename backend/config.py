...
class Settings(BaseModel):
    ...
    missions: list = [
        ...
        },
        {
            ...
            "narrative": "V8.1 在 V8.0 多源共识的基础上引入‘弹性泛化与精度控制’：训练端采用软边界共识（Soft Margin）放宽 occurrence 阈值以覆盖真实海岸带过渡态，同时通过随机森林正则化（如 minLeafPopulation、bagFraction）抑制局部过拟合与碎片化噪点；推理端先进行形态学多数滤波（focal_mode）抹平椒盐碎斑，再叠加海岸带空间围栏（Geofence + clip）靶向聚焦，并透明化水域与内陆（mask 掉 0 和 3），最终输出连贯、可举证的金黄滩涂 vs 鲜红硬化对抗带，用于自然岸线保有率核查与离任审计。",
        ...
        },
        {
            "id": "ch7_beijing",
            "name": "灾害定损 (北方)",
            "title": "京津冀极端暴雨地质灾害定损 (2023)",
            "location": "beijing_2023",
            "api_mode": "ch7_disaster_warning",
            "formula": "AEF Diff × DEM Topology",
            "narrative": "气候变化预警：量化极端强降雨北移引发的次生灾害。AEF 灾后定损大模型已穿透残云。算法提取 2023年7月暴雨前后的 AEF 高维切片，结合 DEM 地形拓扑：在坡度 >12° 区域锁定了植被(A01)断崖式暴跌的滑坡体；在平缓河谷锁定了介电常数(A02)异常涌动的洪泛区，实现了无需人工标注的灾情解构。",
            "camera": {"lat": 39.95, "lon": 115.90, "height": 30000, "duration_s": 4.0},
        },
        {
            "id": "ch7_guangdong",
            "name": "灾害定损 (南方)",
            "title": "中东南部汛期山洪与滑坡预警 (2024)",
            "location": "guangdong_2024",
            "api_mode": "ch7_disaster_warning",
            "formula": "AEF Diff × DEM Topology",
