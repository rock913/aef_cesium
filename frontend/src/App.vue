  ...
  setup() {
    ...
    function _getAlgorithmExplanation(modeId, mission) {
      ...
      if (modeId === 'ch5_coastline_audit') {
        return (
          common +
          `- 第五章·海岸线红线审计（V8.1 泛化与精度控制版）：在 V8.0 多源共识的基础上引入“弹性控制”三件套：软边界共识（Soft Margin）覆盖真实过渡态，随机森林正则化（Tree Regularization）抑制过拟合，形态学多数滤波（Morphological Smoothing）消除椒盐碎斑。\n` +
          `  - 软边界共识：0=水（occ≥80% 且 ESA=水），1=滩涂（5%<occ<80% 且非建筑），2=硬化（ESA=建筑 且 occ≤2%），3=内陆（ESA∈{农田,树林} 且 occ≤2%）。\n` +
          `  - 正则化 RF：通过 minLeafPopulation、bagFraction 等参数避免模型死记硬背单像素噪声，提升未知海域泛化能力。\n` +
          `  - 形态学平滑：推理后先执行 focal_mode 多数滤波，让防波堤/围垦边界更连贯、可读、可举证。\n` +
          `  - 业务约束：再 clip 到海岸带 Geofence，最后透明化水域与内陆（mask 掉 0 和 3），画面只剩“金黄滩涂 vs 鲜红硬化”的连片证据带。\n` +
          `  - 解读建议：将红色锯齿状斑块与红线边界/岸段分区叠加，生成核查点位清单并支撑自然岸线保有率审计。\n`
        )
      }
