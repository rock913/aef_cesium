  ...
  setup() {
    ...
    function _getAlgorithmExplanation(modeId, mission) {
      ...
      if (modeId === 'ch5_coastline_audit') {
        return (
          common +
          `- 第五章·海岸线红线审计（V7.1 Geofenced）：在 V7.0 ESA 金标准弱监督对齐的基础上，增加海岸带空间围栏（Geofence）+ clip，将分析范围物理限定在近岸带，屏蔽内陆建筑/裸土的误触发干扰。\n` +
          `  - 金标准训练：引入欧空局 ESA WorldCover 10m 作为“自动发牌人”，在盐城区域内自动抽取 ESA 标注的纯水体/纯建筑/纯裸土/纯农田像元，交叉采样其 AEF 16 维特征训练随机森林并导出为 Asset。\n` +
          `  - 稳定 ID：由金标准定义且固定（0=水、1=滩涂、2=人工不透水、3=内陆）。\n` +
