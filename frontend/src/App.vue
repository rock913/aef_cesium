  ...
  setup() {
    ...
    function _getAlgorithmExplanation(modeId, mission) {
      ...
      if (modeId === 'ch5_coastline_audit') {
        return (
          common +
          `- 第五章·海岸线红线审计（KD，V6.8 靶向聚类版）：先在 A00/A01/A02（三轴正交：硬度/植被/水分）做 K-Means(k=6) 得到伪标签，再用完整 16 维训练随机森林“背诵”伪标签并导出为 Asset。\n` +
          `  - 工程保障：采样/聚类限制在盐城沿海的更小 training_region，让聚类容量聚焦“浑水/滩涂/硬化”边界，避免深海/深陆挤占簇。\n` +
          `  - 盲盒对齐：首次导出后需按前端颜色确认各类 ID，并对齐 6 槽 palette；同时将“内陆 + 深海”类设为透明 mask（一次对齐后永久稳定）。\n` +
          `  - 解读建议：将红色硬化斑块与红线边界叠加，生成核查点位清单并支撑自然岸线保有率审计。\n`
        )
      }
