  ...
  setup() {
    ...
    function _getAlgorithmExplanation(modeId, mission) {
      ...
      if (modeId === 'ch5_coastline_audit') {
        return (
          common +
          `- 第五章·海岸线红线审计（V8.0 多源共识科学版）：以 JRC 全球水体发生率（occurrence, 0–100）作为水文动力学约束，并与 ESA WorldCover（土地覆盖）交叉印证，构造“只取共识、拒绝模糊”的训练标签，彻底抑制高浑浊水域与潮汐淹没带的语义溢出。\n` +
          `  - 共识标签：0=常年水（occ≥90% 且 ESA=水），1=潮间带滩涂（5%<occ<85% 且 ESA≠建筑），2=人工硬化（ESA=建筑 且 occ=0%），3=内陆背景（ESA∈{农田,树林} 且 occ=0%）；未达共识像元全部剔除，不参与训练。\n` +
          `  - 训练与资产化：在盐城大区分层抽样共识像元，采集其 AEF 16 维特征训练 RandomForest，并导出为 GEE Asset，保证语义可复核与跨期稳定。\n` +
          `  - 双重约束：先 clip 到海岸带围栏，再透明化水域与内陆（mask 掉 0 和 3），画面只剩“金黄滩涂 vs 鲜红硬化”，便于汇报与举证。\n` +
