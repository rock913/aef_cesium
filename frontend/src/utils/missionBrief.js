...
export function buildCommanderBrief(modeId, mission, stats) {
  ...
  if (modeId === 'ch1_yuhang_faceid') {
    ...
    operator = 'KMeans(A00, A02, k=3)'
    brief = '以 AEF 敏感语义特征 (A00 人造物 / A02 水体) 进行固定训练区 KMeans 聚类，强力排除潮汐涨落虚影，快速勾勒违规围填海与潜在越界占用，为涉海环保核查提供先验数字底稿。'
    mechanism = 'KMeans 聚类：提取 A00/A02 两个敏感通道，在固定的盐城近海训练区采样训练 k=3 聚类器，再对目标视域做分类渲染。'
    legends = [
      { color: '#0B1B36', label: '深蓝色斑块：稳定陆地、人工建筑或围垦硬化带' },
      { color: '#E23D28', label: '大面积红色区域：广阔海域与水体' },
      { color: '#F6C431', label: '金黄色地带：潮间带、自然滩涂与淤积区域' },
    ]
    insights = [
    ...
    technicalInsights = [
      '[异动感知] AI 剥离了沿海复杂的潮汐干扰，清晰暴露人工围垦边界与自然沉积滩涂的深浅交错带。',
      '[归因分析] A00/A02 两个通道分别响应硬化结构与含水/水体语义，能把“潮汐虚影”与真实硬化边界分离。',
      '[决策支持] 本审计图谱可直接作为“退堤还滩”评估底版，并服务于涉海党政领导的离任生态审计。',
    ]
