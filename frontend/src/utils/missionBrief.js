...
export function buildCommanderBrief(modeId, mission, stats) {
  ...
  if (modeId === 'ch1_yuhang_faceid') {
    ...
    operator = 'K-Means KD(16-Dim)'
    brief = '面向“离任生态审计/自然岸线保有率”场景：采用 V6.8 靶向聚类版 KD——在更小的海岸过渡带内训练，让 K-Means 的每一簇都服务于“浑水/滩涂/硬化”的精细分离；蒸馏为资产化 RF 后，推理毫秒级稳定不漂移。'
    mechanism = '知识蒸馏（KD，靶向聚类版）：先在 A00/A01/A02（硬度/植被/水分正交维）上做 K-Means(k=6) 生成伪标签，再用完整 16 维训练随机森林蒸馏并导出为 GEE Asset；运行期直接 load+classify。首次导出仍可能出现“盲盒 ID”，通过一次性对齐 palette，并将“内陆 + 深海”类设为透明 mask，保证输出只聚焦海岸带。'
    legends = [
    ...
    technicalInsights = [
      '[稳健推理] KD 固化后，滑动视口不触发聚类重跑，语义不漂移、渲染更稳定。',
      '[物理透视] 低维聚类采用 A00/A01/A02 三轴正交约束，浑水可被单独分簇，不再误判为“红线硬化”。',
      '[靶向空间] 训练区极限收缩到海岸过渡带，避免深海/深陆挤占聚类容量，提升边界分离度。',
      '[盲盒对齐] 首次导出需对齐 palette，并将内陆与深海类设置为透明 mask；一次对齐后永久稳定。',
