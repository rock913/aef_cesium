...
export function buildCommanderBrief(modeId, mission, stats) {
  ...
  } else if (modeId === 'ch9_heritage_deformation') {
    operator = 'NASA ISCE2 + MintPy (Sentinel-1) + 单体 3m 缓冲区五指标归因'
    brief = '把毫米级 InSAR 形变场归因到古建单体：对每栋建筑轮廓做 3 米缓冲区采集相干点，计算最大沉降速率、沉降速率差、角变形等五指标，输出建筑单体相对风险排序。'
    mechanism = 'SBAS-InSAR 时序反演（相干性 ≥0.75 质量控制）→ 单体 3m 缓冲区采点 → 五指标打分（5–9 稳定 / 10–14 中等 / 15–20 优先核查）。输出为 LOS 向相对形变，非结构安全鉴定。'
    legends = [
      { color: '#B03A2E', label: '深红：优先核查（角变形超 1/300 关注线）' },
      { color: '#C85F45', label: '橙红：中等关注（显著差异沉降）' },
      { color: '#C89A3C', label: '黄：轻微关注' },
      { color: '#5DAE8B', label: '青绿：基本稳定（透明）' },
      { color: '#2E9CB8', label: '蓝：微弱抬升' },
    ]
    insights = [
      ...commonInsights,
      '优先核查角变形超 1/300 的台门建筑，与地面巡检台账的墙体开裂记录互证。',
      '输出为相对风险排序，服务修缮排期而非应急鉴定。',
    ]
    technicalInsights = [
      '[单体归因] 古建单体尺度 <30m，Sentinel-1 几十米分辨率下直接取像元采不到足够相干点，故用 3m 缓冲区采点（对标剑川古镇方法）。',
      '[五指标] 最大沉降速率 / 沉降速率差 / 角变形 / 倾斜度 / 时序形变模式，综合打分输出相对风险排序。',
      '[诚实口径] 所有形变值为 LOS 向相对形变（毫米级年速率、厘米级单期位移），不输出结构安全鉴定结论。',
    ]
  } else if (modeId === 'ch9_heritage_wind_risk') {
    operator = 'CMA 智能网格预报 × 风载荷工况知识库 (3000+) × 累计概率'
    brief = '台风逼近前 48 小时，逐小时提取风速风向序列，匹配建筑基本型后检索预计算风载荷知识库，标定屋脊、檐口、翼角等构件级薄弱点，输出灾前专项巡检要点。'
    mechanism = '建筑基本型匹配 → 3000+ 工况离线查表（O(1)）→ 过境窗口内累计受力概率 → 高危部位排序。知识库为预计算结果，现场毫秒级响应，无重计算。'
    legends = [
      { color: '#B03A2E', label: '深红：高风载风险（屋脊/檐口风吸薄弱区）' },
      { color: '#C85F45', label: '橙红：中等风载风险' },
      { color: '#F0C468', label: '黄：一般关注' },
      { color: '#FFF3C4', label: '浅黄：低风险背景' },
    ]
    insights = [
      ...commonInsights,
      '屋脊、檐口为一级风吸薄弱点，灾前优先加固屋面瓦件与翼角。',
      '基本型匹配为群体风险筛查，国宝级建筑需单独仿真（双轨模式）。',
    ]
    technicalInsights = [
      '[预计算知识库] 40+ 基本型 × 70+ 工况维度 × 9–13 级风 × 30° 全风向，共 3000+ 工况全部离线算完，现场秒级查表。',
      '[累计概率] 对过境窗口内逐小时风速风向加权，输出各构件部位累计受力概率分布。',
      '[诚实边界] 基本型匹配是统计意义上的群体筛查，非单体结构鉴定。',
    ]
  } else if (modeId === 'ch9_heritage_aef_discovery') {
    operator = 'AEF Satellite Embedding V1 (64 维 · 10m) + 线性分类器'
    brief = '以浙江已知古建为正样本，直接在 64 维语义嵌入上拟合轻量线性分类器（不微调底座），搬到千里之外的山西盆地全域推理，浮现名录之外的候选古建聚落。'
    mechanism = '64 维嵌入空间拟合线性分类器（k-NN / logistic）→ 10m 网格全域推理 → 连通域聚合 → 剔除已知名录 → 候选线索（非认定结论）。'
    legends = [
      { color: '#3A3416', label: '深褐：低相似背景' },
      { color: '#8A6B1F', label: '褐黄：中相似候选' },
      { color: '#C89A3C', label: '金黄：高相似候选' },
      { color: '#F0C468', label: '亮黄：高置信古建聚落线索' },
    ]
    insights = [
      ...commonInsights,
      '浮现的候选是需要文物部门实地核实的线索，不是认定结论。',
      '证明知识可迁移——不必每到一个新省就从零重来。',
    ]
    technicalInsights = [
      '[零微调] 不微调 AEF 底座，仅在 64 维嵌入上拟合轻量分类器，成本约 2 人日。',
      '[泛化价值] 浙江样本训练的模型直接认出山西古建，是基础模型泛化能力的唯一硬证据。',
      '[诚实报数] 抽样人工核验后如实报准确率，定位为线索而非认定。',
    ]
  } else if (modeId === 'ch9_heritage_change') {
    operator = 'AEF 语义差分 (年际)'
    brief = '相邻两年 AEF 向量点积落差探测古建周边地表突变（新开工、拆改、植被侵占），与 InSAR 沉降速率图做空间交集，回答“哪里在变、为什么变”。'
    mechanism = 'AEF 单位向量点积即余弦相似度，1-dot 即语义变化幅度；变化热点与 InSAR 沉降漏斗空间交集，输出有因果的预警。'
    legends = [
      { color: '#2E9CB8', label: '青蓝：轻微变化' },
      { color: '#C89A3C', label: '黄：中等变化' },
      { color: '#C85F45', label: '橙红：显著变化' },
      { color: '#B03A2E', label: '深红：剧烈变化（周边扰动）' },
    ]
    insights = [
      ...commonInsights,
      '变化热点与 InSAR 沉降漏斗交集，可给出沉降诱因假设（施工降水 vs 自然固结）。',
    ]
    technicalInsights = [
      '[语义差分] AEF 单位向量点积落差探测年际地表突变。',
      '[因果归因] 与 InSAR 沉降场空间交集，把预警从“哪里沉”升级为“为什么沉”。',
    ]
  } else {
