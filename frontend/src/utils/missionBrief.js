...
export function buildCommanderBrief(modeId, mission, stats) {
  ...
  if (modeId === 'ch1_yuhang_faceid') {
    ...
    legends = [
      ...
      { color: '#B000FF', label: '粉红/紫红图斑：鱼骨状开荒带与人类活动区' },
    ...
    operator = 'ZeroShotHyperplane(A00, A02) + PriorityMosaic'
    brief = '采用 A00/A02 的确定性"超平面阈值映射"：建筑硬化带、水体与滩涂由固定规则直接判别，并用优先级合成让建筑层压制水体假信号，确保跨区域（盐城→上海）不发生标签漂移。'
    mechanism = '零样本超平面映射：用 A00(人造物) 与 A02(水/湿度) 的固定阈值规则分类，并以优先级合成（建筑>水体>滩涂）保证语义稳定。'
    legends = [
      { color: '#E23D28', label: '红色：水体/海域（A02 强且 A00 弱）' },
      { color: '#0B1B36', label: '深蓝：建筑/硬化带（A00 强，优先级最高）' },
      { color: '#F6C431', label: '金黄：滩涂/潮间带（A02 弱潮湿且 A00 弱）' },
    ]
    insights = [
      '将硬化带与红线/围填海边界叠加：优先锁定潜在越界占用点。',
      '把结果作为“离任生态审计”底图，输出核查点位与处置清单。',
    ]
    technicalInsights = [
      '[异动感知] 固定阈值规则稳定剥离潮汐/反光干扰，硬化带与自然滩涂边界更清晰。',
      '[归因分析] 跨区域不发生标签漂移：上海高密度建筑不再被误标为“红色水体”。',
      '[决策支持] 输出硬化带/水体/滩涂三类底图，可直接生成红线核查点位与处置清单。',
    ]
