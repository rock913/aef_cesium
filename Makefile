...
.PHONY: help test test-fast test-contract test-integration test-backend-api lint lint-backend lint-frontend test-coverage test-frontend test-frontend-watch \
...
help:
	...
	@echo "  make test-frontend-watch - Run vitest in watch mode"
	@echo "  make lint              - Run backend + frontend linters"
	@echo "  make lint-backend      - Run ruff on backend/"
	@echo "  make lint-frontend     - Run eslint on frontend/src/"
	@echo ""
	@echo "Env toggles:"
...
test-coverage:
	$(PYTEST) --cov=backend --cov-report=html:coverage --cov-report=term
...
lint-backend:
	cd backend && ruff check . 2>/dev/null || echo "Hint: pip install ruff"

lint-frontend:
	cd frontend && npx eslint src/ --quiet 2>/dev/null || echo "Hint: npm install -D eslint"
